...
// object itself when returns is (or includes) bare "object". A bare,
// untyped "array" return with attributes is - in every real-world package
// seen so far - describing each item's shape too, so that's honored as
